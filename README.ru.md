# Scetch — Flutter-приложение для срисовывания через камеру

[English](README.md) | **Русский** | [Тоҷикӣ](README.tg.md)
...
Приложение для Android и iOS на Flutter: camera tracing, photo-to-sketch и наложение изображения на live camera preview для рисования на бумаге. Это экранный референс, без AR world tracking.

**Статус:** исходники Phase 1 созданы, но готовность к production не подтверждена. Генерация Freezed/Drift, golden baseline, Flutter analyzer, тесты и нативные сборки ещё требуют выполнения. Подробнее: [VALIDATION.md](VALIDATION.md).
